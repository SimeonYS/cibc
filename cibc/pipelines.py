...
class CibcPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('cibc.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `cibc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM cibc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO cibc (date, title, link, content)"
